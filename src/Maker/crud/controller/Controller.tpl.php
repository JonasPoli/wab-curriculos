...
<?= $generator->generateRouteForControllerMethod(sprintf($route_path.'/{%s}/edit', $entity_identifier), sprintf('%s_edit', $route_name), ['GET', 'POST']) ?>
    public function edit(Request $request, #[MapEntity(id: '<?= $entity_identifier ?>')] <?= $entity_class_name ?> $<?= $entity_var_singular ?>, EntityManagerInterface $entityManager): Response
    {
        $form = $this->createForm(<?= $form_class_name ?>::class, $<?= $entity_var_singular ?>);
        $form->handleRequest($request);
...
<?= $generator->generateRouteForControllerMethod(sprintf($route_path.'/{%s}', $entity_identifier), sprintf('%s_delete', $route_name), ['POST']) ?>
    public function delete(Request $request, #[MapEntity(id: '<?= $entity_identifier ?>')] <?= $entity_class_name ?> $<?= $entity_var_singular ?>, EntityManagerInterface $entityManager): Response
    {
